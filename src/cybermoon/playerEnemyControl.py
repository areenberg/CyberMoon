...
#the player and enemy control object
    ...
    def entityControl(self,pygame,gameDisplay):
        ...
        if self.enemyList:
            ...
            for enIdx in range(len(self.enemyList)):
                ...
                en.drawEnemy(pygame,gameDisplay,self.getEnemyTarget(pygame,gameDisplay,len(self.enemyList),
                self.player.position(),en))
    ...
    def levelCreater(self,gameDisplay):
        ...
        if self.enTime == 0 and self.enIdx < self.enemyNumber:
            ...
            hsh = math.sqrt((gameDisplay.get_size()[0]*0.5)**2+(gameDisplay.get_size()[1]*0.5)**2)
            ...
            self.enemyList.append( enemy(gameDisplay,[x,y],entype,random.randint(10000,99999)) )
            self.enIdx += 1
    ...
    def gameOverlay(self,pygame,gameDisplay):
        ...
        gameDisplay.blit(wavetxt,(int(displayDim[0]*0.05),int(displayDim[1]*0.04)))

        #enemy count
        enemtxt = ft2.render(("Score " + str(self.score)), False, (192, 192, 192))
        gameDisplay.blit(enemtxt,(int(displayDim[0]*0.80),int(displayDim[1]*0.05)))
    ...
    def getEnemyTarget(self,pygame,gameDisplay,nEnemies,playerPos,enemy):
        ...
        newTarget = playerPos
        eucd = math.sqrt( (enemy.position()[0]-self.avgEnPos[0])**2 + (enemy.position()[1]-self.avgEnPos[1])**2 )
        if eucd < 200:

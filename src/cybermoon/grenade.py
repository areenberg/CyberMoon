...
class grenade:
    ...
    def __init__(self,gameDisplay,startPos,targetPos,genShraps,soundObject):
        ...
        self.startDirection = [random.uniform(0,360) for _ in range(self.numberofShraps)]
        self.initRotationSpeed = [random.uniform(50,65) for _ in range(self.numberofShraps)]
    ...
    def drawGrenade(self,pygame,gameDisplay,playerPosition):
        if self.timer>-1: #count down
            ...
            maxSpeed = 100
            a = 1.1
            self.x = self.x + (maxSpeed*(self.x1-self.x)/displayDim[0])*a**(-self.temp)
            self.y = self.y + (maxSpeed*(self.y1-self.y)/displayDim[0])*a**(-self.temp)
            pygame.draw.circle(gameDisplay, basicFunctions.glow(self.temp,self.lifeTime), [math.floor(self.x),math.floor(self.y)], self.grenadeSize)

        if self.timer == 0: #explosion
            #generate a bunch of shraps
            for s in range(self.numberofShraps):
                self.sh.append(shrap([self.x,self.y],self.startDirection[s],self.initRotationSpeed[s]))

...
class theMainMenu:
    ...
    def __init__(self,pygame,gameDisplay,hsList):
        ...
        self.buttonPositions = [int(displayDim[1]*0.30), int(displayDim[1]*0.42),
         int(displayDim[1]*0.54), int(displayDim[1]*0.66), int(displayDim[1]*0.78)] #button positions in main menu
    ...
    def menuControl(self,pygame,gameDisplay,controller):
        ...
        #DRAW AND COLOR
        #-------------------------------
        gameDisplay.fill(basicFunctions.defineColors("levelBackground"))
        ...
        if not controller.playerAlive: #DISPLAY GAME OVER IF PLAYER IS DEAD
            ...
            pause_txt = self.BigHeader.render('P   A   U   S   E', False, self.pauseHeaderColor())
            ...
            gameDisplay.blit(pause_txt,(int(displayDim[0]*0.33),int(displayDim[1]*0.15)))
            gameDisplay.blit(cont_txt,(int(displayDim[0]*0.1),int(displayDim[1]*0.5)))
            gameDisplay.blit(exit_txt,(int(displayDim[0]*0.1),int(displayDim[1]*0.7)))
            ...
            if event.type == pygame.KEYDOWN:
                if not self.gameOn:
                    if not self.sub_menu:
                        ...
                        if event.key == pygame.K_DOWN: #move the highlighted button (using the keyboard)
    ...
    def updateHighScoreText(self,pygame,event):
        ...
        elif ls<8 and (pygame.K_a <= event.key <= pygame.K_z) or (pygame.K_0 <= event.key <= pygame.K_9):
    ...
    def drawMainMenu(self,pygame,gameDisplay):
        ...
        header_txt = self.BigHeader.render('C  Y  B  E  R   M  O  O  N', False, (100, 100, 100))
        ...
        #display the text
        gameDisplay.blit(header_txt,(int(displayDim[0]*0.2),int(displayDim[1]*0.04)))
        gameDisplay.blit(playgame_txt,(int(displayDim[0]*0.1),self.buttonPositions[0]))
        gameDisplay.blit(highscore_txt,(int(displayDim[0]*0.1),self.buttonPositions[1]))
        gameDisplay.blit(controls_txt,(int(displayDim[0]*0.1),self.buttonPositions[2]))
        gameDisplay.blit(credits_txt,(int(displayDim[0]*0.1),self.buttonPositions[3]))
        gameDisplay.blit(exitgame_txt,(int(displayDim[0]*0.1),self.buttonPositions[4]))
    ...
    def drawControls(self,pygame,gameDisplay):
        ...
        shoot_txt = self.DescriptionFont.render("Left click: Shoot", False, (200, 200, 200))
        pause_txt = self.DescriptionFont.render("Esc: Pause game", False, (200, 200, 200))

        #display the text
        gameDisplay.blit(header_txt,(int(displayDim[0]*0.2),int(displayDim[1]*0.04)))
        gameDisplay.blit(backtomenu_txt,(int(displayDim[0]*0.1),int(displayDim[1]*0.7)))
        gameDisplay.blit(up_txt,(int(displayDim[0]*0.5),int(displayDim[1]*0.25)))
        gameDisplay.blit(down_txt,(int(displayDim[0]*0.5),int(displayDim[1]*0.32)))
        gameDisplay.blit(left_txt,(int(displayDim[0]*0.5),int(displayDim[1]*0.39)))
        gameDisplay.blit(right_txt,(int(displayDim[0]*0.5),int(displayDim[1]*0.46)))
        gameDisplay.blit(shoot_txt,(int(displayDim[0]*0.5),int(displayDim[1]*0.53)))
        gameDisplay.blit(pause_txt,(int(displayDim[0]*0.5),int(displayDim[1]*0.62)))
    ...
    def drawCredits(self,pygame,gameDisplay):
        ...
        #display the text
        dsc1_txt = self.DescriptionFont.render("Concept design: A. R. Andersen", False, (200, 200, 200))
        dsc2_txt = self.DescriptionFont.render("Programming: A. R. Andersen", False, (200, 200, 200))
        dsc3_txt = self.DescriptionFont.render("Sounds: zapsplat.com", False, (200, 200, 200))
        dsc4_txt = self.DescriptionSmallFont.render("This game is based on the PyGame library (pygame.org).", False, (200, 200, 200))


        #display the text
        gameDisplay.blit(header_txt,(int(displayDim[0]*0.2),int(displayDim[1]*0.04)))
        gameDisplay.blit(backtomenu_txt,(int(displayDim[0]*0.1),int(displayDim[1]*0.7)))
        gameDisplay.blit(dsc1_txt,(int(displayDim[0]*0.5),int(displayDim[1]*0.25)))
        gameDisplay.blit(dsc2_txt,(int(displayDim[0]*0.5),int(displayDim[1]*0.32)))
        gameDisplay.blit(dsc3_txt,(int(displayDim[0]*0.5),int(displayDim[1]*0.39)))
        gameDisplay.blit(dsc4_txt,(int(displayDim[0]*0.5),int(displayDim[1]*0.52)))
    ...
    def drawHighScore(self,pygame,gameDisplay):
        ...
        header_txt = self.BigHeader.render('H I G H   S C O R E', False, (100, 100, 100))
        buttontext = ['Back']
        backtomenu_txt = self.ButtonFont.render(buttontext[0], False, (cl[1], 0, 0))
        ...
        for i in range(min(nameCap, len(self.highScoreList.nameList))):
            score_txt = self.DescriptionFont.render(f"{i+1}. {self.highScoreList.nameList[i]}: {self.highScoreList.scoreList[i]}", False, (200, 200, 200))
    ...
    def drawGameOver(self,pygame,gameDisplay,controller):
            ...
            #all other text   
            gameDisplay.fill(basicFunctions.defineColors("lightred"))
            txtGameOver = self.font1.render('GAME OVER', False, (0, 0, 0))
            gameDisplay.blit(txtGameOver,(int(displayDim[0]*0.35),int(displayDim[1]*0.15)))
            #txtGameOverSub = self.font2.render('You were killed!', False, (0, 0, 0))
            
            txtHighScore = self.font2.render('Your name: ' + self.highScoreText + "_", False, (0, 0, 0))
            #gameDisplay.blit(txtGameOverSub,(int(displayDim[0]*0.38),int(displayDim[1]*0.50)))
            gameDisplay.blit(txtHighScore,(int(displayDim[0]*0.38),int(displayDim[1]*0.61)))

            #results
            #txtWave = self.font2.render(("You made it to Wave "+str(controller.gameLevel)), False, (0, 0, 0))
            txtScore = self.font2.render(("Score: "+str(controller.score)), False, (0, 0, 0))
            #gameDisplay.blit(txtWave,(int(displayDim[0]*0.35),int(displayDim[1]*0.73)))
            gameDisplay.blit(txtScore,(int(displayDim[0]*0.40),int(displayDim[1]*0.51)))
            ...
            gameDisplay.blit(txtPosition,(int(displayDim[0]*0.35),int(displayDim[1]*0.35)))
    ...
    def pointingToButton(self,gameDisplay,coord,buttontext):
        ...
        for i in range(len(self.buttonPositions)):
            if coord[1]>(self.buttonPositions[i]) and coord[1]<(self.buttonPositions[i]+34):
                if coord[0]>int(displayDim[0]*0.1) and coord[0]<(int(displayDim[0]*0.1)+len(buttontext[i])*18):
                    button = i + 1
                    self.mouseHighlightMode = True
    ...
    def pointingToButton_pauseMenu(self,gameDisplay,coord):
        ...
        b = 0
        if coord[1]>int(displayDim[1]*0.5) and coord[1]<(int(displayDim[1]*0.5)+34):
            if coord[0]>int(displayDim[0]*0.1) and coord[0]<(int(displayDim[0]*0.1)+len("Continue")*18):
                b = 1
                self.mouseHighlightMode = True
        elif coord[1]>int(displayDim[1]*0.7) and coord[1]<(int(displayDim[1]*0.7)+34):
            if coord[0]>int(displayDim[0]*0.1) and coord[0]<(int(displayDim[0]*0.1)+len("Exit to menu")*18):
                b = 2
                self.mouseHighlightMode = True
    ...
    def pointingToButton_SubMenus(self,gameDisplay,coord):
        ...
        b = 0
        if coord[1]>int(displayDim[1]*0.7) and coord[1]<(int(displayDim[1]*0.7)+34):
            if coord[0]>int(displayDim[0]*0.1) and coord[0]<(int(displayDim[0]*0.1)+len("Back")*18):
                b = 1
                self.mouseHighlightMode = True
    ...
    def animateShraps(self,pygame,gameDisplay):
        ...
        if not self.menuShraps:
            for i in range(random.randint(1,10)):
                startPos = [displayDim[0]*random.uniform(0.5,0.99),displayDim[1]*random.uniform(0.3,0.99)]
